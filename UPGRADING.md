...
| ------------------------------------------------------- | ------------- | --------------------- | ------------------------------------- |
| [v4.13.0 / React v0.11.0](#upgrading-to-v4130--react-v0110) | April 2026    | No breaking changes | 🟢 Low - adopt the public React footer if needed |
...
## Upgrading to v4.13.0 / React v0.11.0
...
- Toolkit consumers can now keep the default footer support links text-only while opting into shared link-icon cues with footer data when a directional or external hint adds value.
- Toolkit consumers can also explicitly hide the small-print line with `smallPrint` while continuing to support the legacy `copyright` alias when no migration is needed.
...
3. Re-run visual QA at desktop, tablet, and mobile if you have local footer overrides, because the spacing and wrapping behavior now follows the current footer spec more closely.
