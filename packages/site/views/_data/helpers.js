...
const toolkitComponentsPath = path.join(
  __dirname,
  "../../../toolkit/components/",
);
...
exports.getNunjucksCode = (filepath) => {
  ...
  const content = fileContents.replace(/{%\s*extends\s*\S*\s*%}\s+/, "");
...
exports.getHTMLCode = (filepath) => {
  const fileContents = getFileContents(filepath);

  let html;
  try {
    html = env.renderString(fileContents);
  } catch (err) {
    if (err) {
      // eslint-disable-next-line no-console
      console.log(`Could not get HTML code from ${filepath}`, err);
    }
  }
