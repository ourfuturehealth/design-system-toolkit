...
- `layout`: `fixed` or `fluid`. Defaults to `fixed`.
- `showBottomBorder`: optional boolean. Defaults to `true`.
...
- `account`: optional account state. Use either `{ type: 'sign-in', href }` or `{ type: 'account', accountHref, signOutHref }`. Default labels are `Log in` and `Log out`.
...
- The mobile menu button reuses existing button styling patterns. Dark uses the inverted bordered treatment, while light uses the standard outlined treatment.
